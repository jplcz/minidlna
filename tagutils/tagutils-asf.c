...
static inline uint32_t le32_to_cpu(uint32_t le32) {
#if __BYTE_ORDER == __LITTLE_ENDIAN
  return le32;
#else
  uint32_t be32 = ((le32 << 24) & 0xff000000) | ((le32 << 8) & 0x00ff0000) |
                  ((le32 >> 8) & 0x0000ff00) | ((le32 >> 24) & 0x000000ff);
  return be32;
#endif
}

static inline uint64_t le64_to_cpu(uint64_t le64) {
#if __BYTE_ORDER == __LITTLE_ENDIAN
  return le64;
#else
  uint64_t be64;
...
static inline uint32_t cpu_to_be32(uint32_t cpu32) {
#if __BYTE_ORDER == __LITTLE_ENDIAN
  uint32_t be32 = ((cpu32 << 24) & 0xff000000) | ((cpu32 << 8) & 0x00ff0000) |
                  ((cpu32 >> 8) & 0x0000ff00) | ((cpu32 >> 24) & 0x000000ff);
  return be32;
#else
  return cpu32;
#endif
...
static inline uint8_t fget_byte(FILE *fp) {
  uint8_t d;
...
static inline uint16_t fget_le16(FILE *fp) {
  uint16_t d;
...
static inline uint32_t fget_le32(FILE *fp) {
  uint32_t d;
...
static int utf16le_to_utf8(char *dst, int n, uint16_t utf16le) {
...
static int _asf_read_file_properties(FILE *fp, asf_file_properties_t *p,
                                     uint32_t size) {
  size_t len;
...
static void _pick_dlna_profile(struct song_metadata *psong, uint16_t format) {
  /* DLNA Profile Name */
...
static int _asf_read_audio_stream(FILE *fp, struct song_metadata *psong,
                                  size_t size) {
...
static int _asf_read_media_stream(FILE *fp, struct song_metadata *psong,
                                  uint32_t size) {
  ...
  if (IsEqualGUID(&s.MajorType, &ASF_MediaTypeAudio) &&
      IsEqualGUID(&s.FormatType, &ASF_FormatTypeWave) &&
      s.FormatSize >= sizeof(wfx)) {
...
static int _asf_read_stream_object(FILE *fp, struct song_metadata *psong,
                                   size_t size) {
...
static int _asf_read_extended_stream_object(FILE *fp,
...
static int _asf_read_header_extension(FILE *fp, struct song_metadata *psong,
                                      uint32_t size) {
  off_t pos;
  long off;
...
static int _asf_load_string(FILE *fp, int type, size_t size, char *buf,
                            size_t len) {
...
static void *_asf_load_picture(FILE *fp, size_t size, void *bm, int *bm_size) {
  size_t i;
  char buf[256];
#if 0
	//
  ...
  if (!strcasecmp(buf, "image/jpeg") || !strcasecmp(buf, "image/jpg") ||
...
static int _get_asffileinfo(const char *file, struct song_metadata *psong) {
  ...
  while (NumObjects > 0) {
    ...
    if (IsEqualGUID(&tmp.ID, &ASF_FileProperties)) {
      ...
      if (_asf_load_string(fp, ASF_VT_UNICODE, AuthorLength, buf,
                           sizeof(buf))) {
        if (buf[0])
      ...
      while (NumEntries > 0) {
        ...
        if (!strcasecmp(buf, "AlbumTitle") ||
            !strcasecmp(buf, "WM/AlbumTitle")) {
          if (_asf_load_string(fp, ValueType, ValueLength, buf, sizeof(buf)))
            if (buf[0])
              psong->album = strdup(buf);
        } else if (!strcasecmp(buf, "AlbumArtist") ||
                   !strcasecmp(buf, "WM/AlbumArtist")) {
          if (_asf_load_string(fp, ValueType, ValueLength, buf, sizeof(buf))) {
            if (buf[0])
              psong->contributor[ROLE_ALBUMARTIST] = strdup(buf);
          }
        } else if (!strcasecmp(buf, "Description") ||
                   !strcasecmp(buf, "WM/Track")) {
          if (_asf_load_string(fp, ValueType, ValueLength, buf, sizeof(buf)))
            if (buf[0])
        ...
        } else if (!strcasecmp(buf, "WM/Picture") &&
                   (ValueType == ASF_VT_BYTEARRAY)) {
          psong->image = (uint8_t *)_asf_load_picture(
              fp, ValueLength, psong->image, &psong->image_size);
        } else if (!strcasecmp(buf, "TrackNumber") ||
                   !strcasecmp(buf, "WM/TrackNumber")) {
          if (_asf_load_string(fp, ValueType, ValueLength, buf, sizeof(buf)))
            if (buf[0])
              psong->track = atoi(buf);
