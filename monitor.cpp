...
static char *check_nfo(const char *path) {
  char file[PATH_MAX];

  strncpyt(file, path, sizeof(file));
  strip_ext(file);

  return sql_get_text_field(
      db,
...
int monitor_insert_file(const char *name, const char *path) {
  ...
  len = strlen(path) + 1;
  if (!(path_buf = (char *)malloc(len)) || !(last_dir = (char *)malloc(len)) ||
  ...
  if (!depth) {
    ...
    int ret = insert_file(name, path, id + 2,
                          get_next_available_id("OBJECTS", id), dir_types);
    if (ret == 1 && (mtype & TYPE_PLAYLIST)) {
      next_pl_fill =
          time(NULL) + 120; // Schedule a playlist scan for 2 minutes from now.
      // DEBUG DPRINTF(E_MAXDEBUG, L_INOTIFY,  "Playlist scan scheduled for %s",
      // ctime(&next_pl_fill));
    }
    sqlite3_free(id);
  }
...
static bool check_notsparse(const char *path)
...
int monitor_insert_directory(int fd, char *name, const char *path) {
  ...
  if (sql_get_int_field(db, "SELECT ID from DETAILS where PATH = '%q'", path) >
    ...
    id = sql_get_text_field(db,
                            "SELECT OBJECT_ID from OBJECTS o left join DETAILS "
