...
int insert_playlist(const char *path, const char *name) {
  ...
  objname = strdup(name);
  strip_ext(objname);
  matches = sql_get_int_field(
      db, "SELECT count(*) from PLAYLISTS where NAME = '%q'", objname);
  if (matches > 0) {
    char *newname;
...
static unsigned int gen_dir_hash(const char *path) {
...
int fill_playlists(void) {
  ...
  for (i = 3; i < rows * 3; i++) {
    ...
    while (next_plist_track(&plist, &file, NULL, type) == 0) {
      ...
      if (sql_get_int_field(
              db, "SELECT 1 from OBJECTS where OBJECT_ID = '%s$%llX$%d'",
      ...
      if (last_dir) {
        if (hash == last_hash) {
          ...
          detailID = sql_get_int_field(
              db, "SELECT ID from DETAILS where PATH = '%q/%q'", last_dir,
              fname);
        } else
          detailID = -1;
        if (detailID <= 0) {
    ...
    retry:
      ...
      detailID = sql_get_int_field(
          db, "SELECT ID from DETAILS where PATH like '%%%q'", fname);
      if (detailID > 0) {
      found:
        ...
        sql_exec(
            ...
            "SELECT"
            " '%s$%llX$%d', '%s$%llX', CLASS, DETAIL_ID, NAME, OBJECT_ID from "
            "OBJECTS"
            " where DETAIL_ID = %lld and OBJECT_ID glob '" BROWSEDIR_ID "$*'",
            MUSIC_PLIST_ID, plID, plist.track, MUSIC_PLIST_ID, plID, detailID);
        if (!last_dir) {
          last_dir = sql_get_text_field(
              db, "SELECT PATH from DETAILS where ID = %lld", detailID);
          if (last_dir) {
    ...
    sql_exec(db, "UPDATE PLAYLISTS set FOUND = %d where ID = %lld", found,
             plID);
  }
done:
