...
// _aac_findatom:
static long _aac_findatom(FILE *fin, long max_offset, const char *which_atom,
                          int *atom_size) {
...
static int _get_aactags(const char *file, struct song_metadata *psong) {
  ...
  if (atom_offset != -1) {
    while (current_offset < atom_length) {
      ...
      if (fread(current_data, 1, (size_t)(current_size - 8), fin) !=
          (size_t)(current_size - 8))
        break;

      current_data[current_size - 8] = '\0';
      if (!memcmp(current_atom,
                  "\xA9"
                  "nam",
                  4))
        psong->title = strdup((char *)&current_data[16]);
      else if (!memcmp(current_atom,
                       "\xA9"
                       "ART",
                       4) ||
               !memcmp(current_atom,
...
// aac_lookforatom
static off_t _aac_lookforatom(FILE *aac_fp, const char *atom_path,
                              unsigned int *atom_length) {
  long atom_offset;
  off_t file_size;
  ...
  while (cur_p) {
    ...
    atom_offset =
        _aac_findatom(aac_fp, file_size, atom_name, (int *)atom_length);
    if (atom_offset == -1) {
      return -1;
    }
    cur_p = strchr(cur_p, ':');
...
int _aac_check_extended_descriptor(FILE *infile) {
  short int i;
  unsigned char buf[3];
  ...
  if (fread((void *)&buf, 1, 3, infile) < 3)
    return -1;
  for (i = 0; i < 3; i++) {
    if ((buf[i] != 0x80) && (buf[i] != 0x81) && (buf[i] != 0xFE)) {
...
// _get_aacfileinfo
int _get_aacfileinfo(const char *file, struct song_metadata *psong) {
  FILE *infile;
  long atom_offset;
  int atom_length;
  int sample_size;
  ...
  // move to 'mvhd' atom
  atom_offset =
      _aac_lookforatom(infile, "moov:mvhd", (unsigned int *)&atom_length);
  if (atom_offset != -1) {
  ...
  atom_offset = _aac_lookforatom(infile, "moov:trak:mdia:minf:stbl:stsd:alac",
                                 (unsigned int *)&atom_length);
  if (atom_offset != -1) {
  ...
  atom_offset = _aac_lookforatom(infile, "moov:trak:mdia:minf:stbl:stsd:mp4a",
                                 (unsigned int *)&atom_length);
  if (atom_offset != -1) {
    fseek(infile, atom_offset + 32, SEEK_SET);
    if (fread(buffer, sizeof(unsigned char), 2, infile) == 2)
    ...
    atom_offset =
        _aac_findatom(infile, atom_length - (ftell(infile) - atom_offset),
                      "esds", &atom_length);
    ...
    if (atom_offset != -1) {
      ...
      if (!fread((void *)&buffer, 1, 1, infile) || (buffer[0] != 0x03) ||
          (_aac_check_extended_descriptor(infile) != 0))
        goto bad_esds;
      fseek(infile, 4, SEEK_CUR);
      if (!fread((void *)&buffer, 1, 1, infile) || (buffer[0] != 0x04) ||
          (_aac_check_extended_descriptor(infile) != 0))
        goto bad_esds;
      fseek(infile, 10, SEEK_CUR); // 10 bytes into section 4 should be average
                                   // bitrate.  max bitrate is 6 bytes in.
      if (fread((void *)&bitrate, sizeof(unsigned int), 1, infile))
        psong->bitrate = ntohl(bitrate);
      if (!fread((void *)&buffer, 1, 1, infile) || (buffer[0] != 0x05) ||
          (_aac_check_extended_descriptor(infile) != 0))
        goto bad_esds;
      fseek(infile, 1,
            SEEK_CUR); // 1 bytes into section 5 should be the setup data
      if (fread((void *)&buffer, 2, 1, infile)) {
        profile_id =
            (aac_object_type_t)(buffer[0] >> 3); // first 5 bits of setup data
                                                 // is the Audo Profile ID
