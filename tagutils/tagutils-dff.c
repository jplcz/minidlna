 ...
 */
#define GET_DFF_INT64(p)                                                       \
  ((((uint64_t)((p)[0])) << 56) | (((uint64_t)((p)[1])) << 48) |               \
   (((uint64_t)((p)[2])) << 40) | (((uint64_t)((p)[3])) << 32) |               \
   (((uint64_t)((p)[4])) << 24) | (((uint64_t)((p)[5])) << 16) |               \
   (((uint64_t)((p)[6])) << 8) | (((uint64_t)((p)[7]))))

#define GET_DFF_INT32(p)                                                       \
  ((((uint32_t)((p)[0])) << 24) | (((uint32_t)((p)[1])) << 16) |               \
   (((uint32_t)((p)[2])) << 8) | (((uint32_t)((p)[3]))))

#define GET_DFF_INT16(p) ((((uint16_t)((p)[0])) << 8) | (((uint16_t)((p)[1]))))

static int _get_dfffileinfo(const char *file, struct song_metadata *psong) {
  FILE *fp;
  uint32_t len;
  uint32_t rt;
    ...
    if (strncmp((char *)ckbuf, "PROP", 4) == 0) {
      ...
      if (!(rt = fread(dsdsdckData, len, 1, fp))) {
        DPRINTF(E_WARN, L_SCANNER,
                "Could not read DSD/DST Sound Data chunk from %s\n", file);
        fclose(fp);
        return -1;
      }
      ...
      if (strncmp((char *)dsdsdckData, "DSD ", 4) == 0) {
        ...
        psong->song_length =
            (int)((double)dsdsdckDataSize / (double)samplerate /
                  (double)channels * 8 * 1000);
        ...
        if (!(rt = fread(frteckData, 18, 1, fp))) {
          DPRINTF(E_WARN, L_SCANNER,
                  "Could not read DST Frame Information chunk from %s\n", file);
          fclose(fp);
          return -1;
        }
        ...
        if (strncmp((char *)frteckData, "FRTE", 4) == 0) {
          ...
          DPRINTF(E_WARN, L_SCANNER,
                  "Invalid DST Frame Information chunk in %s\n", file);
          fclose(fp);
          return -1;
        }
        ...
        if (!(rt = fread(dstickData, 12, 1, fp))) {
          if (ferror(fp)) {
            DPRINTF(E_WARN, L_SCANNER,
                    "Could not read DST Sound Index chunk from %s\n", file);
            fclose(fp);
            return -1;
          } else {
            // EOF
      ...
      if (!(rt = fread(diinckData.data(), diinckDataSize, 1, fp))) {
        DPRINTF(E_WARN, L_SCANNER,
                "Could not read Edited Master Information chunk from %s\n",
                file);
        fclose(fp);
        return -1;
      }
      ...
      while (icount < diinckDataSize) {
        if (!strncmp((char *)diinckData.data() + icount, "EMID", 4)) {
          ...
          strncpy((char *)arttext.data(), (char *)diinckData.data() + icount,
                  sizeof(arttext) - 1);
          ...
          strncpy((char *)titletext.data(), (char *)diinckData.data() + icount,
                  sizeof(titletext) - 1);
  ...
  // int)totalsize); DPRINTF(E_DEBUG, L_SCANNER, "propckDataSize is 0x%016lx\n",
  // (long unsigned int)propckDataSize); DPRINTF(E_DEBUG, L_SCANNER,
  // "cmprckDataSize is 0x%016lx\n", (long unsigned int)cmprckDataSize);
  // DPRINTF(E_DEBUG, L_SCANNER, "abssckDataSize is 0x%016lx\n", (long unsigned
  // int)abssckDataSize); DPRINTF(E_DEBUG, L_SCANNER, "lscockDataSize is
  // 0x%016lx\n", (long unsigned int)lscockDataSize); DPRINTF(E_DEBUG,
  // L_SCANNER, "dsdsdckDataSize is 0x%016lx\n", (long unsigned
  // int)dsdsdckDataSize); DPRINTF(E_DEBUG, L_SCANNER, "dstickDataSize is
  // 0x%016lx\n", (long unsigned int)dstickDataSize); DPRINTF(E_DEBUG,
  // L_SCANNER, "comtckDataSize is 0x%016lx\n", (long unsigned
  // int)comtckDataSize); DPRINTF(E_DEBUG, L_SCANNER, "diinckDataSize is
  // 0x%016lx\n", (long unsigned int)diinckDataSize); DPRINTF(E_DEBUG,
  // L_SCANNER, "diarckDataSize is 0x%016lx\n", (long unsigned
  // int)diarckDataSize); DPRINTF(E_DEBUG, L_SCANNER, "ditickDataSize is
  // 0x%016lx\n", (long unsigned int)ditickDataSize); DPRINTF(E_DEBUG,
  // L_SCANNER, "manfckDataSize is 0x%016lx\n", (long unsigned
  // int)manfckDataSize);

  // DPRINTF(E_DEBUG, L_SCANNER, "Got dff fileinfo successfully=%s\n", file);
  // DEBUG DPRINTF(E_DEBUG, L_SCANNER, "TITLE is %s\n",psong->title );
  // DEBUG DPRINTF(E_DEBUG, L_SCANNER, "ARTIST is
  // %s\n",psong->contributor[ROLE_ARTIST]); DEBUG DPRINTF(E_DEBUG, L_SCANNER,
  // "samplerate is  %d\n", psong->samplerate); DEBUG DPRINTF(E_DEBUG,
  // L_SCANNER, "song_length is  %d\n", psong->song_length); DEBUG
  // DPRINTF(E_DEBUG, L_SCANNER, "channels are  %d\n", psong->channels); DEBUG
