...
static unsigned int next_highest(unsigned int num) {
  num |= num >> 1;
  num |= num >> 2;
  num |= num >> 4;
  num |= num >> 8;
...
static void raise_watch_limit(unsigned int limit) {
...
int monitor_add_watch(int fd, const char *path) {
  struct watch *nw;
  int wd;
...
int monitor_remove_watch(int fd, const char *path) {
  struct watch *w;
...
static int inotify_create_watches(int fd) {
  FILE *max_watches;
...
static int inotify_remove_watches(int fd) {
  struct watch *w = watches;
  struct watch *last_w;
  int rm_watches = 0;
        ...
        if (event->mask & IN_ISDIR &&
          ...
          if ((event->mask & (IN_MOVED_TO | IN_CREATE)) &&
            ...
            DPRINTF(E_DEBUG, L_INOTIFY, "The %s link %s was %s.\n",
                    (S_ISLNK(st.st_mode) ? "symbolic" : "hard"), path_buf,
                    (event->mask & IN_MOVED_TO ? "moved here" : "created"));
