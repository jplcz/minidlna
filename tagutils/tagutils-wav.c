...
#define GET_WAV_INT32(p)                                                       \
  ((((uint8_t)((p)[3])) << 24) | (((uint8_t)((p)[2])) << 16) |                 \
   (((uint8_t)((p)[1])) << 8) | (((uint8_t)((p)[0]))))

#define GET_WAV_INT16(p) ((((uint8_t)((p)[1])) << 8) | (((uint8_t)((p)[0]))))

static int _get_wavtags(const char *filename, struct song_metadata *psong) {
  int fd;
  uint32_t len;
  ...
  while (current_offset + 8 < (size_t)psong->file_size) {
    ...
    // DEBUG DPRINTF(E_DEBUG, L_SCANNER, "Read block %02x%02x%02x%02x (%c%c%c%c)
    // of "
    ...
    if (strncmp((char *)&hdr, "fmt ", 4) == 0) {
      ...
      // DEBUG DPRINTF(E_DEBUG,L_SCANNER,"Compression code:
      // %d\n",compression_code); DEBUG DPRINTF(E_DEBUG,L_SCANNER,"Channel
      // count:    %d\n",channel_count); DEBUG DPRINTF(E_DEBUG,L_SCANNER,"Sample
      // Rate:      %d\n",sample_rate); DEBUG DPRINTF(E_DEBUG,L_SCANNER,"Sample
      // bit length %d\n",sample_bit_length);
      ...
      if (read(fd, tags, len) < len || strncmp(tags, "INFO", 4) != 0) {
        free(tags);
        goto next_block;
      }
      tags[len] = '\0';
      ...
      while (off < len - 8) {
        ...
        if (taglen > 2048) {
          ...
          m = &(psong->title);
        else if (strncmp(p, "IALB", 4) == 0 || strncmp(p, "IPRD", 4) == 0)
          m = &(psong->album);
        else if (strncmp(p, "IGRE", 4) == 0 || strncmp(p, "IGNR", 4) == 0)
          m = &(psong->genre);
        ...
        else if (strncmp(p, "ICOM", 4) == 0 || strncmp(p, "IMUS", 4) == 0)
          m = &(psong->contributor[ROLE_COMPOSER]);
        else if (strncasecmp(p, "ITRK", 4) == 0)
          psong->track = atoi(p + 8);
        else if (strncmp(p, "ICRD", 4) == 0 || strncmp(p, "IYER", 4) == 0)
          psong->year = atoi(p + 8);
        if (m) {
  ...
  if (((format_data_length != 16) && (format_data_length != 18)) ||
      (compression_code != 1) || (channel_count < 1)) {
    DPRINTF(E_WARN, L_SCANNER, "Invalid wav header in %s\n", filename);
    return -1;
  }
  ...
  /* Upon further review, WAV files should be little-endian, and DLNA requires
  the LPCM profile to be big-endian. asprintf(&(psong->mime),
  "audio/L16;rate=%d;channels=%d", psong->samplerate, psong->channels);
  */

  return 0;
}

static int _get_wavfileinfo(const char *, struct song_metadata *psong) {
  psong->lossless = 1;
  /* Upon further review, WAV files should be little-endian, and DLNA requires
  the LPCM profile to be big-endian. asprintf(&(psong->dlna_pn), "LPCM");
  */
