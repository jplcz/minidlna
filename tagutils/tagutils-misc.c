...
typedef enum { ICONV_OK, ICONV_TRYNEXT, ICONV_FATAL } iconv_result;

#ifdef HAVE_ICONV
static iconv_result do_iconv(const char *to_ces, const char *from_ces,
...
#else  // HAVE_ICONV
static iconv_result do_iconv(const char *, const char *, char *, size_t, char *,
                             size_t) {
  return ICONV_FATAL;
}
#endif // HAVE_ICONV
...
} iconv_map[] = {{"ja_JP", {"CP932", "CP950", "CP936", "ISO-8859-1", 0}},
...
static int _lang2cp(const char *lang) {
  int cp;
...
static unsigned char *_get_utf8_text(const id3_ucs4_t *native_text) {
  ...
  i = lang_index;
  // (1) try utf8 -> default
  rc = do_iconv(iconv_map[i].cpnames[0], "UTF-8", in8, strlen(in8), iconv_buf,
                MAX_ICONV_BUF);
  if (rc == ICONV_OK) {
    ...
    // (2) try default -> utf8
    rc = do_iconv("UTF-8", iconv_map[i].cpnames[0], in, strlen(in), iconv_buf,
                  MAX_ICONV_BUF);
    if (rc == ICONV_OK) {
      ...
      for (n = 1; n < N_LANG_ALT && iconv_map[i].cpnames[n]; n++) {
        rc = do_iconv("UTF-8", iconv_map[i].cpnames[n], in, strlen(in),
...
static void vc_scan(struct song_metadata *psong, const char *comment,
                    const size_t length) {
  char strbuf[1024];
  ...
  // Xiph.org lists recommended field names for interoperability between
  // programs. Beyond these software may use other tag names, and because the
  // files we are tasked with reading may come from a variety of sources we
  // include other commonly used tags where we can.
  // https://xiph.org/vorbis/doc/v-comment.html
  // ALBUM, ARTIST, PUBLISHER, COPYRIGHT, DISCNUMBER, ISRC, EAN/UPN, LABEL,
  // LABELNO, LICENSE, OPUS, SOURCEMEDIA, TITLE, TRACKNUMBER, VERSION,
  // ENCODED-BY, ENCODING,
  ...
  // -- In addition, some software (e.g. Windows Media Player) insists on using
  // YEAR
  // -- rather than DATE, so support this where DATE is not available for
  ...
  if (!strncasecmp(strbuf, "ALBUM=", 6)) {
    ...
    if (length >= (5 + 10) && isdigit(strbuf[5 + 0]) &&
        isdigit(strbuf[5 + 1]) && ispunct(strbuf[5 + 2]) &&
        isdigit(strbuf[5 + 3]) && isdigit(strbuf[5 + 4]) &&
        ispunct(strbuf[5 + 5]) && isdigit(strbuf[5 + 6]) &&
        isdigit(strbuf[5 + 7]) && isdigit(strbuf[5 + 8]) &&
        isdigit(strbuf[5 + 9])) {
      // nn-nn-yyyy
      strbuf[5 + 10] = '\0';
