...
static int _get_dsftags(const char *file, struct song_metadata *psong) {
...
static int _get_dsffileinfo(const char *file, struct song_metadata *psong) {
  FILE *fp;
  ...
  // psong->samplerate); DEBUG DPRINTF(E_DEBUG, L_SCANNER, "song_length is
  // %d\n", psong->song_length); DEBUG DPRINTF(E_DEBUG, L_SCANNER, "channels are
  // %d\n", psong->channels); DEBUG DPRINTF(E_DEBUG, L_SCANNER, "samplecount are
  // %lld\n", samplecount);
  fclose(fp);
