 ...
 * There are function to effiently resize a JPEG image, and some utility
 * functions. They are here to allow loading and saving JPEG data directly to or
 * from memory with libjpeg. The standard functions only allow you to read from
 * or write to a file.
 *
 * The reading code comes from the JpgAlleg library, at
 * http://wiki.allegro.cc/index.php?title=Libjpeg The writing code was posted on
 * a Google group from openjpeg, at
 * http://groups.google.com/group/openjpeg/browse_thread/thread/331e6cf60f70797f
 * The resize functions come from the resize_image project, at
 * http://www.golac.fr/Image-Resizer
 */
...
#define COL_FULL(red, green, blue, alpha)                                      \
...
static void term_source(j_decompress_ptr) { return; }

void jpeg_memory_src(j_decompress_ptr cinfo, const unsigned char *buffer,
                     size_t bufsize) {
  struct my_src_mgr *src;

  if (!cinfo->src)
    cinfo->src = (jpeg_source_mgr *)(*cinfo->mem->alloc_small)(
        (j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof(struct my_src_mgr));
  ;
...
std::shared_ptr<image_s> image_new_from_jpeg(const char *path, int is_file,
  ...
  vimage =
      (rotate & (ROTATE_90 | ROTATE_270)) ? image_new(h, w) : image_new(w, h);
  if (!vimage) {
    jpeg_destroy_decompress(&cinfo);
    if (is_file)
      fclose(file);
  ...
  if (cinfo.output_components == 3) {
    int rx, ry;
    ofs = 0;
    if ((ptr = (unsigned char *)malloc(w * 3 * cinfo.rec_outbuf_height + 16)) ==
        NULL) {
      DPRINTF(E_WARN, L_METADATA, "malloc failed\n");
      jpeg_destroy_decompress(&cinfo);
      if (is_file)
    ...
    for (y = 0; y < h; y += cinfo.rec_outbuf_height) {
      ...
      for (x = 0; x < w * cinfo.rec_outbuf_height; x++) {
        ...
        ofs =
            (rotate & (ROTATE_90 | ROTATE_270)) ? ry + (rx * h) : rx + (ry * w);
        if (ofs < maxbuf)
          vimage->buf[ofs] =
              COL(ptr[x + x + x], ptr[x + x + x + 1], ptr[x + x + x + 2]);
      }
    }
    free(ptr);
    ...
    for (y = 0; y < h; y += cinfo.rec_outbuf_height) {
      ...
      for (i = 0; i < cinfo.rec_outbuf_height; i++) {
        for (x = 0; x < w; x++) {
          rx = (rotate & (ROTATE_180 | ROTATE_270)) ? (x - w + 1) * -1 : x;
          ofs = (rotate & (ROTATE_90 | ROTATE_270)) ? ry + (rx * h)
                                                    : rx + (ry * w);
          if (ofs < maxbuf)
...
void image_upsize(image_s *pdest, image_s *psrc, int32_t width,
                  int32_t height) {
  int32_t vx, vy;
...
void image_downsize(image_s *pdest, image_s *psrc, int32_t width,
                    int32_t height) {
  int32_t vx, vy;
  pix vcol;
  int32_t i, j;
  ...
  for (vy = 0; vy < height; vy++) {
    for (vx = 0; vx < width; vx++) {
      ...
      put_pix_alpha_replace(pdest, vx, vy,
                            COL_FULL((uint8_t)red, (uint8_t)green,
