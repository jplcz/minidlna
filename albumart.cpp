...
void update_if_album_art(const char *path) {
  ...
  while ((dp = readdir(dh)) != NULL) {
    ...
    if (((is_video(dp->d_name) && (dir_type & TYPE_VIDEO)) ||
      ...
      ret = sql_exec(db,
                     "UPDATE DETAILS set ALBUM_ART = %lld where PATH = '%q' "
                     "and ALBUM_ART != %lld",
                     (long long)art_id, file, (long long)art_id);
      if (ret == SQLITE_OK)
...
char *check_embedded_art(const char *path, uint8_t *image_data,
                         int image_size) {
...
int64_t find_album_art(const char *path, uint8_t *image_data, int image_size) {
  ...
  if ((image_size &&
    ...
    ret = sql_get_int_field(db, "SELECT ID from ALBUM_ART where PATH = '%q'",
                            album_art);
    if (!ret) {
      if (sql_exec(db, "INSERT into ALBUM_ART (PATH) VALUES ('%q')",
                   album_art) == SQLITE_OK)
