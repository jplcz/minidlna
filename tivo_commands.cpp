...
static void SendRootContainer(struct upnphttp *h) {
  char *resp;
  int len;
...
static void SendFormats(struct upnphttp *h, const char *sformat) {
  char *resp;
  int len;
...
static char *tivo_unescape_tag(char *tag) {
  modifyString(tag, "&amp;amp;", "&amp;", 1);
  modifyString(tag, "&amp;amp;lt;", "&lt;", 1);
  modifyString(tag, "&amp;lt;", "&lt;", 1);
  modifyString(tag, "&amp;amp;gt;", "&gt;", 1);
...
static int callback(void *args, int argc, char **argv, char **azColName) {
  ...
  char *id = argv[0], *class = argv[1], *detailID = argv[2], *size = argv[3],
       *title = argv[4], *duration = argv[5], *bitrate = argv[6],
       *sampleFrequency = argv[7], *artist = argv[8], *album = argv[9],
       *genre = argv[10], *comment = argv[11], *date = argv[12],
       *resolution = argv[13], *mime = argv[14];
  ...
  if (strncmp(class, "item", 4) == 0) {
    ...
    if (strncmp(mime, "audio", 5) == 0) {
      ...
      if (date) {
        ...
        strcatf(str, "<CaptureDate>0x%X</CaptureDate>",
      ...
      if (comment)
        strcatf(str, "<Description>%s</Description>",
                tivo_unescape_tag(comment));
    } else {
      return 0;
    }
    ...
    if (duration) {
      strcatf(str, "<Duration>%d</Duration>",
              atoi(strrchr(duration, '.') + 1) +
                  (1000 * atoi(strrchr(duration, ':') + 1)) +
                  (60000 * atoi(strrchr(duration, ':') - 2)) +
                  (3600000 * atoi(duration)));
    }
    if (bitrate) {
    ...
    strcatf(str,
            ...
            "<Url>/%s/%s.%s</Url>%s"
            "</Content>",
            mime, (flags & FLAG_SEND_RESIZED) ? "Resized" : "MediaItems",
...
#ifdef __sparc__ /* Adding filters on large containers can take a long time on \
                    slow processors */
    count = sql_get_int_field(
        db, "SELECT count(*) from OBJECTS where PARENT_ID = '%s'", id);
#else
    count = sql_get_int_field(
        db,
        "SELECT count(*) from OBJECTS o left join DETAILS d on (d.ID = "
        "o.DETAIL_ID) where PARENT_ID = '%s' and "
        " (MIME in ('image/jpeg', 'audio/mpeg', 'video/mpeg', "
        "'video/x-tivo-mpeg', 'video/x-tivo-mpeg-ts')"
        " or CLASS glob 'container*')",
        id);
#endif
    strcatf(str,
            "<Item>"
...
static void SendItemDetails(struct upnphttp *h, int64_t item) {
  char *sql;
  char *zErrMsg = NULL;
  ...
  str.size = 32768;
  str.off =
      sprintf(str.data, "<?xml version='1.0' encoding='UTF-8' ?>\n<TiVoItem>");
  args.str = &str;
  args.requested = 1;
  xasprintf(&sql,
            SELECT_COLUMNS
            "from OBJECTS o left join DETAILS d on (d.ID = o.DETAIL_ID)"
            " where o.DETAIL_ID = %lld group by o.DETAIL_ID",
            (long long)item);
...
static void SendContainer(struct upnphttp *h, const char *objectID,
                          int itemStart, int itemCount, char *anchorItem,
                          int anchorOffset, int recurse, char *sortOrder,
                          char *filter, unsigned long int randomSeed) {
  ...
  if (strlen(objectID) == 1) {
    ...
    item = sql_get_text_field(
        db, "SELECT NAME from OBJECTS where OBJECT_ID = '%q'", objectID);
    if (item) {
  ...
  if (filter) {
    ...
    strcpy(myfilter, "MIME in ('image/jpeg', 'audio/mpeg', 'video/mpeg', "
                     "'video/x-tivo-mpeg', 'video/x-tivo-mpeg-ts') or CLASS "
                     "glob 'container*'");
  }
  ...
  if (anchorItem) {
    ...
    sql = sqlite3_mprintf(
        "SELECT %s from OBJECTS o left join DETAILS d on (o.DETAIL_ID = d.ID)"
        " where %s and (%s)"
        " %s"
        " order by %s",
        what, which, myfilter, groupBy, order2);
    DPRINTF(E_DEBUG, L_TIVO, "%s\n", sql);
