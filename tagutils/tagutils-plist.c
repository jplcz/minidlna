...
static int (*_next_track)(struct song_metadata *, struct stat *, char *,
                          char *);
static int _m3u_pls_next_track(struct song_metadata *, struct stat *, char *,
                               char *);

int start_plist(const char *path, struct song_metadata *psong,
                struct stat *stat, char *, char *type) {
  char *fname, *suffix;
...
int _m3u_pls_next_track(struct song_metadata *psong, struct stat *, char *,
                        char *type) {
  char buf[MAX_BUF], *p;
  int len;
...
int next_plist_track(struct song_metadata *psong, struct stat *stat, char *lang,
                     char *type) {
  if (_next_track)
