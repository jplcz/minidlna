...
static ogg_stream_set *_ogg_create_stream_set(void) {
  ogg_stream_set *set = (ogg_stream_set *)calloc(1, sizeof(ogg_stream_set));

  set->streams =
      (ogg_stream_processor *)calloc(5, sizeof(ogg_stream_processor));
  set->allocated = 5;
  set->used = 0;
...
static void _ogg_vorbis_process(ogg_stream_processor *stream, ogg_page *page,
    ...
    if (inf->doneheaders < 3) {
      ...
      if (inf->doneheaders == 3) {
        ...
        if (inf->vc.comments > 0)
          DPRINTF(E_MAXDEBUG, L_SCANNER, "User comments section follows...\n");
...
static void _ogg_vorbis_end(ogg_stream_processor *stream,
                            struct song_metadata *psong) {
  ogg_misc_vorbis_info *inf = (ogg_misc_vorbis_info *)stream->data;
  double bitrate, time;
...
static void _ogg_process_null(ogg_stream_processor *, ogg_page *,
                              struct song_metadata *) {
  // invalid stream
}

static void _ogg_process_other(ogg_stream_processor *stream, ogg_page *page,
                               struct song_metadata *) {
  ogg_stream_pagein(&stream->os, page);
}

static void _ogg_free_stream_set(ogg_stream_set *set) {
  int i;
...
static int _ogg_streams_open(ogg_stream_set *set) {
  int i;
  int res = 0;
...
static void _ogg_null_start(ogg_stream_processor *stream) {
  stream->process_end = NULL;
  stream->type = "invalid";
  stream->process_page = _ogg_process_null;
}

static void _ogg_other_start(ogg_stream_processor *stream, const char *type) {
  if (type)
    stream->type = type;
  else
    stream->type = "unknown";
...
static void _ogg_vorbis_start(ogg_stream_processor *stream) {
...
static ogg_stream_processor *_ogg_find_stream_processor(ogg_stream_set *set,
                                                        ogg_page *page) {
  ...
  if (set->allocated < set->used)
    ...
    set->allocated += 5;
    set->streams = (ogg_stream_processor *)realloc(
        set->streams, sizeof(ogg_stream_processor) * set->allocated);
    stream = &set->streams[set->used];
  }
  set->used++;
...
static int _ogg_get_next_page(FILE *f, ogg_sync_state *sync, ogg_page *page,
                              ogg_int64_t *written) {
  int ret;
  char *buffer;
  int bytes;
  ...
  while ((ret = ogg_sync_pageout(sync, page)) <= 0) {
    if (ret < 0)
      DPRINTF(E_WARN, L_SCANNER,
              "Hole in data found at approximate offset %lld bytes. Corrupted "
              "ogg.\n",
              (long long)*written);
...
static int _get_oggfileinfo(const char *filename, struct song_metadata *psong) {
  ...
  while (_ogg_get_next_page(file, &sync, &page, &written)) {
    ...
    if (p->isnew) {
      DPRINTF(E_MAXDEBUG, L_SCANNER,
              ...
              p->serial, p->type);
      if (!p->start)
        DPRINTF(E_WARN, L_SCANNER, "stream start flag not set on stream %d\n",
                p->num);
    } else if (p->start)
      DPRINTF(E_WARN, L_SCANNER,
