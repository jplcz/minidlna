...
int64_t GetAudioMetadata(const char *path, const char *name) {
  ...
  } else if (ends_with(path, ".flac") || ends_with(path, ".fla") ||
             ends_with(path, ".flc")) {
    strcpy(type, "flc");
  ...
  ret = sql_exec(
      ...
      "  TITLE, CREATOR, ARTIST, ALBUM, GENRE, COMMENT, DISC, TRACK, DLNA_PN, "
      "MIME, ALBUM_ART) "
      "VALUES"
      " (%Q, %lld, %lld, '%s', %d, %d, %d, %Q, %Q, %Q, %Q, %Q, %Q, %Q, %d, %d, "
      "%Q, '%s', %lld);",
      path, (long long)file.st_size, (long long)file.st_mtime, m.duration,
      song.channels, song.bitrate, song.samplerate, m.date, m.title, m.creator,
      m.artist, m.album, m.genre, m.comment, song.disc, song.track, m.dlna_pn,
      song.mime ? song.mime : m.mime, album_art);
  if (ret != SQLITE_OK) {
...
int64_t GetImageMetadata(const char *path, const char *name) {
  ...
  if (e || (e = exif_content_get_entry(ed->ifd[EXIF_IFD_EXIF],
  ...
  if (e) {
    ...
    if (e) {
      ...
      if (!strcasestr(model, make))
        snprintf(model, sizeof(model), "%s %s", make,
...
no_exifdata:
  /* If SOF parsing fails, then fall through to reading the JPEG data with
   * libjpeg to get the resolution */
  if (image_get_jpeg_resolution(path, &width, &height) != 0 || !width ||
      !height) {
    infile = fopen(path, "r");
    if (infile) {
  ...
  ret = sql_exec(db,
                 ...
                 path, m.title, (long long)file.st_size,
                 (long long)file.st_mtime, m.date, m.resolution, m.rotation,
                 thumb, m.creator, m.dlna_pn, m.mime);
  if (ret != SQLITE_OK) {
    DPRINTF(E_ERROR, L_METADATA, "Error inserting details for '%s'!\n", path);
    ret = 0;
  } else {
...
int64_t GetVideoMetadata(const char *path, const char *name) {
  ...
  for (i = 0; i < (int)ctx->nb_streams; i++) {
    if (lav_codec_type(ctx->streams[i]) == AVMEDIA_TYPE_AUDIO &&
      ...
      continue;
               ...
               !lav_is_thumbnail_stream(ctx->streams[i], &m.thumb_data,
                                        &m.thumb_size) &&
               video_stream == -1) {
      video_stream = i;
    ...
    case AV_CODEC_ID_AAC:
      ...
      case AAC_LC_ER:
        ...
        if (lav_channels(astream) <= 2 && lav_bit_rate(astream) <= 576000)
          ...
          DPRINTF(E_DEBUG, L_METADATA,
                  "Unhandled AAC: %lld channels, %lld bitrate\n",
  ...
  if (vstream) {
    ...
    /* NOTE: The DLNA spec only provides for ASF (WMV), TS, PS, and MP4
     * containers. Skip DLNA parsing for everything else. */
    if (strcmp(ctx->iformat->name, "avi") == 0) {
      ...
      if (lav_codec_id(vstream) == AV_CODEC_ID_MPEG4) {
        ...
        if (memcmp(fourcc, "XVID", 4) == 0 || memcmp(fourcc, "DX50", 4) == 0 ||
    ...
    case AV_CODEC_ID_MPEG2VIDEO:
      ...
      if (strcmp(ctx->iformat->name, "mpegts") == 0) {
        ...
        DPRINTF(E_DEBUG, L_METADATA,
                "Stream %d of %s is %s MPEG2 TS packet size %d\n", video_stream,
                basepath, m.resolution, raw_packet_size);
        ...
        DPRINTF(E_DEBUG, L_METADATA,
                ...
                basepath, ctx->iformat->name, m.resolution);
        free(m.dlna_pn);
        m.dlna_pn = NULL;
      }
      break;
      ...
      if (strcmp(ctx->iformat->name, "mpegts") == 0) {
        ...
        if (((((lav_width(vstream) == 1920 || lav_width(vstream) == 1440) &&
            ...
            ((lav_width(vstream) == 1280 && lav_height(vstream) == 720) &&
             fps == 59 && !interlaced)) {
          if ((lav_profile(vstream) == FF_PROFILE_H264_MAIN ||
               lav_profile(vstream) == FF_PROFILE_H264_HIGH) &&
              audio_profile == PROFILE_AUDIO_AC3) {
            ...
            lav_profile(vstream) = FF_PROFILE_SKIP;
                     ...
                     lav_height(vstream) == 1080) ||
                    (lav_width(vstream) == 1440 &&
                     lav_height(vstream) == 1080) ||
                    (lav_width(vstream) == 1280 &&
                     lav_height(vstream) == 720) ||
                    (lav_width(vstream) == 720 &&
        ...
        case FF_PROFILE_H264_CONSTRAINED_BASELINE:
          ...
          if (lav_width(vstream) <= 352 && lav_height(vstream) <= 288 &&
            ...
            break;
          } else if (lav_width(vstream) <= 352 && lav_height(vstream) <= 288 &&
                     lav_bit_rate(vstream) <= 3000000) {
            off += sprintf(m.dlna_pn + off, "CIF30_");
            break;
          }
          /* Fall back to Main Profile if it doesn't match a Baseline DLNA
             profile. */
          else
            off -= 3;
          [[fallthrough]];
        ...
        case FF_PROFILE_H264_MAIN:
          ...
          if (lav_width(vstream) <= 720 && lav_height(vstream) <= 576 &&
        ...
        case FF_PROFILE_H264_HIGH:
          ...
          if (lav_width(vstream) <= 1920 && lav_height(vstream) <= 1152 &&
              lav_bit_rate(vstream) <= 30000000 &&
              audio_profile == PROFILE_AUDIO_AC3) {
            off += sprintf(m.dlna_pn + off, "HD_");
          } else {
            DPRINTF(E_DEBUG, L_METADATA,
                    "Unsupported h.264 HP video profile! [%lldbps, %d audio : "
                    "%s]\n",
        ...
        case FF_PROFILE_H264_CONSTRAINED_BASELINE:
          if (lav_width(vstream) <= 352 && lav_height(vstream) <= 288) {
            ...
            if (lav_level(vstream) == 30 &&
                audio_profile == PROFILE_AUDIO_AAC && ctx->bit_rate <= 5000000)
            ...
            if (lav_level(vstream) <= 31 &&
                audio_profile == PROFILE_AUDIO_AAC && ctx->bit_rate <= 15000000)
              off += sprintf(m.dlna_pn + off, "BL_L31_HD_AAC");
        ...
        mp4_mp_fallback:
          ...
          break;
        case FF_PROFILE_H264_HIGH:
          if (lav_width(vstream) <= 1920 && lav_height(vstream) <= 1080 &&
    ...
    case AV_CODEC_ID_MPEG4:
      ...
      DPRINTF(E_DEBUG, L_METADATA, "Stream %d of %s is MPEG4 [%c%c%c%c/0x%X]\n",
              video_stream, basepath, isprint(fourcc[0]) ? fourcc[0] : '_',
              isprint(fourcc[1]) ? fourcc[1] : '_',
              isprint(fourcc[2]) ? fourcc[2] : '_',
              isprint(fourcc[3]) ? fourcc[3] : '_', lav_codec_tag(vstream));
      ...
      if (strcmp(ctx->iformat->name, "mov,mp4,m4a,3gp,3g2,mj2") == 0) {
        ...
        if (ends_with(path, ".3gp")) {
          ...
          default:
            DPRINTF(E_DEBUG, L_METADATA,
                    "No DLNA profile found for MPEG4-P2 3GP/%d file %s\n",
          ...
          } else if (ctx->bit_rate <= 4000000 && lav_width(vstream) <= 640 &&
                     lav_height(vstream) <= 480 &&
                     audio_profile == PROFILE_AUDIO_AAC) {
            off += sprintf(m.dlna_pn + off, "MP4_SP_VGA_AAC");
          } else {
            DPRINTF(E_DEBUG, L_METADATA,
                    "Unsupported h.264 video profile! [%dx%d, %lldbps]\n",
                    lav_width(vstream), lav_height(vstream),
    ...
    case AV_CODEC_ID_WMV3:
      /* I'm not 100% sure this is correct, but it works on everything I could
       * get my hands on */
    ...
    case AV_CODEC_ID_VC1:
      ...
      if ((lav_width(vstream) <= 176) && (lav_height(vstream) <= 144) &&
          (lav_level(vstream) == 0)) {
        ...
        default:
          ...
          break;
                 ...
                 (lav_profile(vstream) == 0) && (ctx->bit_rate / 8 <= 384000)) {
  ...
  if (strcmp(ctx->iformat->name, "asf") == 0) {
    ...
    if (ctx->metadata) {
      ...
      while (
          (tag = av_dict_get(ctx->metadata, "", tag, AV_DICT_IGNORE_SUFFIX))) {
...
video_no_dlna:
  ...
  ret = sql_exec(db,
                 "INSERT into DETAILS"
                 " (PATH, SIZE, TIMESTAMP, DURATION, DATE, CHANNELS, BITRATE, "
                 "SAMPLERATE, RESOLUTION,"
                 "  TITLE, CREATOR, ARTIST, GENRE, COMMENT, DLNA_PN, MIME, "
                 "ALBUM_ART, DISC, TRACK) "
                 "VALUES"
                 " (%Q, %lld, %lld, %Q, %Q, %u, %u, %u, %Q, '%q', %Q, %Q, %Q, "
                 "%Q, %Q, '%q', %lld, %u, %u);",
                 path, (long long)file.st_size, (long long)file.st_mtime,
                 m.duration, m.date, m.channels, m.bitrate, m.frequency,
                 m.resolution, m.title, m.creator, m.artist, m.genre, m.comment,
                 m.dlna_pn, m.mime, album_art, m.disc, m.track);
  if (ret != SQLITE_OK) {
    DPRINTF(E_ERROR, L_METADATA, "Error inserting details for '%s'!\n", path);
    ret = 0;
  } else {
