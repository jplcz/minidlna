...
static int _get_flctags(const char *filename, struct song_metadata *psong) {
  ...
  do {
    ...
    case FLAC__METADATA_TYPE_STREAMINFO:
      ...
      psong->bitrate =
          (((uint64_t)(psong->file_size) * 1000) / (psong->song_length / 8));
    ...
    case FLAC__METADATA_TYPE_VORBIS_COMMENT:
      for (i = 0; i < block->data.vorbis_comment.num_comments; i++) {
        vc_scan(psong, (char *)block->data.vorbis_comment.comments[i].entry,
...
static int _get_flcfileinfo(const char *, struct song_metadata *psong) {
  psong->lossless = 1;
  psong->vbr_scale = 1;
