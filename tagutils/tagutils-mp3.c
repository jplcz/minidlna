    ...
    if (!strcmp(pid3frame->id, "YTCP")) /* for id3v2.2 */
    {
      ...
      if ((strcmp((char *)id3_field_getlatin1(&pid3frame->fields[1]),
          ...
          (strcmp((char *)id3_field_getlatin1(&pid3frame->fields[1]),
                  "image/jpg") == 0) ||
          (strcmp((char *)id3_field_getlatin1(&pid3frame->fields[1]), "jpeg") ==
           0)) {
...
static int _decode_mp3_frame(unsigned char *frame, struct mp3_frameinfo *pfi) {
  ...
  if (pfi->layer == 1)
    pfi->frame_length =
        (12 * pfi->bitrate * 1000 / pfi->samplerate + pfi->padding) * 4;
  else
    pfi->frame_length =
        144 * pfi->bitrate * 1000 / pfi->samplerate + pfi->padding;
...
static void _mp3_get_average_bitrate(FILE *infile, struct mp3_frameinfo *pfi,
  ...
  if (fread(frame_buffer, 1, sizeof(frame_buffer), infile) !=
      sizeof(frame_buffer))
    return;

  while (!found) {
    while ((frame_buffer[index] != 0xFF) &&
           (index < (int)(sizeof(frame_buffer) - 4)))
      index++;
      ...
      if (!_decode_mp3_frame(frame_buffer, &fi)) {
        ...
        if (cbr && (frames > 100)) {
          DPRINTF(E_DEBUG, L_SCANNER,
                  "File appears to be CBR... quitting frame "
                  "_mp3_get_frame_count()\n");
          return;
        }
      }
...
static int _get_mp3fileinfo(const char *file, struct song_metadata *psong) {
  ...
  if (strncmp((char *)pid3->id, "ID3", 3) == 0) {
    ...
    /* found an ID3 header... */
    size = (pid3->size[0] << 21 | pid3->size[1] << 14 | pid3->size[2] << 7 |
            pid3->size[3]);
    fp_size = size + sizeof(struct id3header);
    first_check = 1;

    snprintf(tagversion, sizeof(tagversion), "ID3v2.%d.%d", pid3->version[0],
             pid3->version[1]);
  ...
  while (!found) {
    ...
    if ((n_read = fread(buffer, 1, sizeof(buffer), infile)) <
    ...
    while (!found) {
      ...
      if (!_decode_mp3_frame(&buffer[index], &fi)) {
        if (!strncasecmp((char *)&buffer[index + fi.xing_offset + 4], "XING",
                         4)) {
          ...
          if (!found) {
            // cannot find second frame. Song may be too short. So assume first
            // frame is valid.
            found = 1;
            fp_size += index;
          }
  ...
  if (fread(id3v1taghdr, 1, 4, infile) == 4) {
    if (id3v1taghdr[0] == 'T' && id3v1taghdr[1] == 'A' &&
        id3v1taghdr[2] == 'G') {
  ...
  if (!psong->song_length) {
    if (fi.number_of_frames) {
      psong->song_length =
          (int)((double)(fi.number_of_frames * fi.samples_per_frame * 1000.) /
                (double)fi.samplerate);
      psong->vbr_scale = 78;
    } else {
      psong->song_length =
          (int)((double)(file_size - fp_size) * 8. / (double)fi.bitrate);
    }
  }
  psong->channels = fi.stereo ? 2 : 1;

  fclose(infile);
  // DEBUG DPRINTF(E_INFO, L_SCANNER, "Got fileinfo successfully for file=%s
