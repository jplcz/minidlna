...
unsigned long long monotonic_us(void) {
  ...
  const auto usec = std::chrono::time_point_cast<std::chrono::microseconds>(pt);
...
int read_bootid_node(unsigned char *buf, size_t size) {
  ...
  if ((fseek(boot_id, 24, SEEK_SET) < 0) ||
      (fscanf(boot_id, "%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx", &buf[0], &buf[1],
              &buf[2], &buf[3], &buf[4], &buf[5]) != 6)) {
    fclose(boot_id);
    return -1;
  }
...
static void read_random_bytes(unsigned char *buf, size_t size) {
  ...
  try {
    ...
    std::generate_n(buf, size, [&]() { return dist(rng); });
  } catch (...) {
