...
char *modifyString(char *string, const char *before, const char *after,
  ...
  if (newlen > oldlen) {
    ...
    s = (char *)realloc(string,
                        strlen(string) + ((newlen - oldlen) * chgcnt) + 1);
...
int is_album_art(const char *name) {
  ...
  for (album_art_name = album_art_names; album_art_name;
    ...
    if (album_art_name->wildcard) {
      if (strncmp(album_art_name->name, name, strlen(album_art_name->name)) ==
          0)
        break;
    } else {
  ...
  if (lstat(path, &entry) == 0) {
    if (S_ISLNK(entry.st_mode)) {
      if ((len = readlink(path, str_buf, PATH_MAX - 1)) > 0) {
        str_buf[len] = '\0';
        // DEBUG DPRINTF(E_DEBUG, L_GENERAL, "Checking for recursive symbolic
        // link: %s (%s)\n", path, str_buf);
